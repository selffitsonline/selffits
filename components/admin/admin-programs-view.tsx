...
export function AdminProgramsView() {
  ...
  const getCourseTimings = (course: any): string[] => {
    if (!course) return [];
    if (Array.isArray(course.availableTimings) && course.availableTimings.length > 0) {
      return course.availableTimings;
    }
    if (Array.isArray(course.schedule?.availableTimings) && course.schedule.availableTimings.length > 0) {
      return course.schedule.availableTimings;
    }
    if (course.schedule?.timing) return [course.schedule.timing];
    const combined = [
      ...(course.schedule?.morningBatch || []),
      ...(course.schedule?.eveningBatch || []),
    ];
    if (combined.length > 0) return combined;
    return ["03:30 PM to 04:15 PM (GMT)", "06:00 PM to 06:45 PM (GMT)"];
  };

  const handleAddTiming = (newTiming: string) => {
    if (!editingCourse || !newTiming.trim()) return;
    const currentTimings = getCourseTimings(editingCourse);
    if (currentTimings.includes(newTiming.trim())) return;

    const updatedTimings = [...currentTimings, newTiming.trim()];
    const updatedCourse = {
      ...editingCourse,
    ...
    if (activeTab === "mma") {
      setMmaData((prev: any) => ({
        ...
        [mmaSubCat]: {
          ...prev[mmaSubCat],
          courses: prev[mmaSubCat].courses.map((c: any) => (c.id === editingCourse.id ? updatedCourse : c)),
        },
      }));
    } else {
      setHiitData((prev: any[]) => prev.map((c: any) => (c.id === editingCourse.id ? updatedCourse : c)));
    }
  };

  const handleDeleteTiming = (idx: number) => {
    if (!editingCourse) return;
    const currentTimings = getCourseTimings(editingCourse);
    const updatedTimings = currentTimings.filter((_, i) => i !== idx);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className={`${editingCourse ? "lg:col-span-5" : "lg:col-span-12"} space-y-4`}>
            {displayedCourses.map((c: any) => (
              ...
              >
                ...
                {/* Available Timings Badges */}
                <div className="mt-3 pt-3 border-t border-white/10 space-y-1">
                  <span className="text-[10px] font-extrabold uppercase text-gray-400 tracking-wider flex items-center gap-1">
                    <Clock className="w-3 h-3 text-[#0080FF]" /> Available Class Timings:
                  </span>
                  <div className="flex flex-wrap gap-1">
                    {getCourseTimings(c).map((tStr: string, tIdx: number) => (
                      <span
                        key={tIdx}
                        className="px-2 py-0.5 rounded-md bg-[#0080FF]/15 text-[#0080FF] border border-[#0080FF]/30 text-[10px] font-bold"
                      >
                        {tStr}
                      </span>
                    ))}
          ...
          {editingCourse && (
            <div className="lg:col-span-7 bg-[#14161D] border border-white/10 rounded-2xl p-6 space-y-6 shadow-2xl sticky top-24">
              ...
              {/* 5. Available Class Timings Management */}
              <div className="space-y-3 p-4 rounded-xl bg-[#0F1117] border border-white/5">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-bold uppercase text-white flex items-center gap-1.5">
                    <Clock className="w-4 h-4 text-[#0080FF]" /> Available Class Timings List
                  </label>
                </div>

                <p className="text-[11px] text-gray-400">
                  Configure available daily class timing slots displayed to prospective students.
                </p>

                {/* Timing Chips */}
                <div className="flex flex-wrap gap-2">
                  {getCourseTimings(editingCourse).map((timingStr, tIdx) => (
                    <div
                      key={tIdx}
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-[#0080FF]/20 text-[#0080FF] border border-[#0080FF]/30 text-xs font-bold"
                    >
                      <span>{timingStr}</span>
                      <button
                        type="button"
                        onClick={() => handleDeleteTiming(tIdx)}
                ...
                <div className="pt-2 space-y-2 border-t border-white/5">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {[
                      ...
                      <button
                        key={preset}
                        type="button"
                        onClick={() => handleAddTiming(preset)}
                        className="px-2.5 py-1 rounded-lg bg-white/5 hover:bg-white/10 text-gray-300 text-[11px] font-semibold transition-all border border-white/10 cursor-pointer"
                      >
                        + {preset}
                      </button>
                    ))}
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    <input
                      type="text"
                      id="customTimingInput"
                      placeholder="Or type custom timing (e.g. 10:00 AM to 11:00 AM EST)"
                      onKeyDown={(e) => {
                        if (e.key === "Enter") {
                          e.preventDefault();
                          handleAddTiming(e.currentTarget.value);
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        const el = document.getElementById("customTimingInput") as HTMLInputElement;
                        if (el && el.value) {
                          handleAddTiming(el.value);
                          el.value = "";
                        }
                      }}
                      className="px-3.5 py-2 rounded-lg bg-[#0080FF] hover:bg-[#0066CC] text-white text-xs font-extrabold transition-all shrink-0 cursor-pointer"
                    >
                      Add Timing
                    </button>
                  </div>
